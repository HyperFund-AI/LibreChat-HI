...
async function runAgentToolLoop({
  ...
  for (let i = 0; i < maxTurns; i++) {
    ...
    if (apiTools.length > 0) {
      const toolInstructions = apiTools
        .map((t) => {
    ...
    if (toolUseBlocks.length > 0) {
      ...
      if (response.stop_reason === 'end_turn') {
        ...
        const isStrict =
          toolChoice === 'any' || (typeof toolChoice === 'object' && toolChoice.type === 'any');
      ...
      for (const block of toolUseBlocks) {
        ...
        if (submissionToolName && toolName === submissionToolName) {
          ...
          if (executor) {
            ...
            // Handle interruption tool
            const question = toolInput.question;
            logger.info(`[${agentName}] Asking user: ${question}`);

            // We do NOT execute anything locally. We pause.
            // Add the tool use to history, but NOT the result (the result will be injected when resuming)
            // Actually, we should probably return here immediately.

            // Break the loop and return special status
            return {
              result: {
                status: 'PAUSED',
                question,
                toolCallId: toolId, // Needed to resume later
              },
              messages: history,
            };
          }
