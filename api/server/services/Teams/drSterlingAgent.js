...
const parseTeamFromMarkdown = (markdownOutput) => {
  try {
    ...
    if (!tableMatch) {
      tableMatch = markdownOutput.match(
        /##\s*TEAM COMPOSITION[^\n]*\n+([^\n]*Tier[^\n]*\n[\s\S]*?)(?=\n##|$)/i,
      );
      if (tableMatch) {
    ...
    if (tableMatch) {
      logger.info(
        `[parseTeamFromMarkdown] Found table with delimiter: "${delimiter === '\t' ? 'TAB' : 'PIPE'}"`,
      );
    ...
    const specSection = markdownOutput.match(
      /## SUPERHUMAN SPECIFICATIONS([\s\S]*?)(?=##\s*PROJECT INTEGRATION|##\s*COLLABORATION|##\s*ORCHESTRATION|$)/i,
    );
    if (specSection) {
      ...
      logger.info(
        `[parseTeamFromMarkdown] Found ${memberHeaders.length} member headers in specifications section`,
      );
      ...
      for (let i = 0; i < memberHeaders.length; i++) {
        ...
        const endIndex =
          i < memberHeaders.length - 1 ? memberHeaders[i + 1].index : specContent.length;
        ...
        if (existingMember) {
          ...
          updatedCount++;
          logger.debug(
            `[parseTeamFromMarkdown] Updated member: ${existingMember.name} (${fullInstructions.length} chars)`,
          );
        } else {
          ...
          team.members.push({
            ...
            behavioralLevel:
              block.match(/\*\*Behavioral Science Level:\*\*\s*([^\n]+)/i)?.[1]?.trim() || 'NONE',
          });
          addedCount++;
          logger.debug(
            `[parseTeamFromMarkdown] Added member from specs: ${nameOnly} (${fullInstructions.length} chars)`,
          );
        }
      }

      logger.info(
        `[parseTeamFromMarkdown] Specifications: Updated ${updatedCount} members, Added ${addedCount} new members`,
      );
    }
...
const mergeTeamMembers = (parsedTeams) => {
  ...
  logger.info(
    `[mergeTeamMembers] Merged ${parsedTeams.length} team specs into ${mergedTeam.members.length} unique members`,
  );
  return mergedTeam;
};
...
const convertParsedTeamToAgents = (parsedTeam, conversationId) => {
  ...
  return parsedTeam.members.map((member, index) => {
    ...
    const instructions =
      member.instructions || `You are ${member.name}, a ${member.role}. ${member.expertise || ''}`;

    logger.debug(
      `[convertParsedTeamToAgents] Converting ${member.name}: instructions length = ${instructions.length} chars`,
    );
